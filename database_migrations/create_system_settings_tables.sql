-- Database Schema for System Settings Management
...
CREATE TABLE IF NOT EXISTS system_logs (
    id INT PRIMARY KEY AUTO_INCREMENT,
    log_type ENUM('error', 'warning', 'info', 'debug', 'security', 'system', 'user', 'backup', 'settings') NOT NULL DEFAULT 'info',
...
'<h1>Low Stock Alert</h1><p>The following products are running low on stock:</p>{{products_table}}<p>Please restock soon.</p>',
'The following products are running low on stock:\n\n{{products_list}}\n\nPlease restock soon.',
'["products_table", "products_list"]'),

('system_maintenance', 'System Maintenance', 
'<h1>System Maintenance</h1><p>The system will be under maintenance from {{start_time}} to {{end_time}}.</p><p>During this time, the system will be unavailable.</p>',
'The system will be under maintenance from {{start_time}} to {{end_time}}.\n\nDuring this time, the system will be unavailable.',
'["start_time", "end_time"]);

-- Insert default feature toggles
INSERT INTO feature_toggles (feature_name, feature_group, is_enabled, description, requires_restart) VALUES
...
CREATE INDEX idx_system_settings_group_key ON system_settings(setting_group, setting_key);
CREATE INDEX idx_backup_history_created_type ON backup_history(created_at, backup_type);
CREATE INDEX idx_system_logs_created_type ON system_logs(created_at, log_type);
CREATE INDEX idx_email_templates_active ON email_templates(is_active);
CREATE INDEX idx_feature_toggles_group_enabled ON feature_toggles(feature_group, is_enabled);
...
-- Create view for enabled features
CREATE VIEW enabled_features AS
SELECT feature_name, feature_group, description, requires_restart
